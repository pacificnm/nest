---
name: nest-react-circularprogress
description: Use when working with the CircularProgress React component from @nest/components – an animated spinner or determinate indicator for loading, uploads, and progress tracking.
---
...
<div className="relative">
  ...
  <span className="absolute inset-0 flex items-center justify-center text-xs">60%</span>
</div>
```
...
<CircularProgress variant="determinate" value={50} aria-label="Upload progress" />
```

## Styling

- Uses `text-*` color classes for theming.
- Uses `animate-spin` for indeterminate animation.
- Uses `transition-all duration-300 ease-out` for determinate progress.
...
1. Use indeterminate for unknown duration.
2. Use determinate when progress is measurable.
3. Provide context with labels or surrounding text.
4. Consider size appropriate to use case.
5. Match brand colors where relevant.
