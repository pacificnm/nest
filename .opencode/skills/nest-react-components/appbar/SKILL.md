---
name: nest-react-appbar
description: Use when working with the AppBar component from @nest/components – a desktop‑style header bar.
---

# AppBar
Top application bar for desktop‑window chrome (title bar, menu row, toolbar). Exported together with `Toolbar`.

## When to Use
- The persistent header of a desktop app window: title, menu bar, and toolbar actions.
- Pair with `Toolbar` for the flex row of title/actions, or with `MenuBar` for a File/Edit‑style dropdown row.
...
## Props
`AppBar` and `Toolbar` both extend native HTML attributes (`className`, `children`, …). See the tables above for component‑specific props.
