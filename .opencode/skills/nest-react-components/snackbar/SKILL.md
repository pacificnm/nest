---
name: nest-react-snackbar
description: Use when working with the Snackbar React component from @nest/components – a transient toast for brief feedback.
---
...
### With Undo Action
```tsx
<Snackbar open={open} onClose={() => setOpen(false)} action={
  <Button size="small" onClick={handleUndo}>Undo</Button>
}>Item deleted</Snackbar>
```
### Different Severities
```tsx
<Snackbar open={open} onClose={() => setOpen(false)} severity="success">Saved successfully!</Snackbar>
<Snackbar open={open} onClose={() => setOpen(false)} severity="error">Failed to save</Snackbar>
```
### Custom Position
```tsx
<Snackbar open={open} onClose={() => setOpen(false)} position="top-right">Notification in top-right</Snackbar>
```
## Accessibility
